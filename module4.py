...
class Bank:
    ...
    def create(self, name, bal, fd):
        ...
        accno = None
        while True:
            accno = ''.join(map(str, random.sample(range(0, 10), 6))) 
            check_query = "SELECT EXISTS(SELECT 1 FROM bankdata WHERE accNo = %s)"  # Check if new accno already exists
            try:
                with self.db.cursor() as cursor:
                    cursor.execute(check_query, (accno,))
                    if cursor.fetchone()[0] == 0:  # If account number does not exist
                        break 
                self.db.commit()
            except connector.Error as e:
                print(e)
    ...
    def car(self,accno,car_value,model_year):  
        query = "SELECT SUM(amount) AS trans_amt FROM banktrans WHERE accNo = %s"
        update_balance = "UPDATE bankdata SET balance = balance + %s WHERE accNo = %s"
        new_balance = "SELECT balance FROM bankdata WHERE accNo = %s"
        update_trans = "INSERT INTO banktrans (accNo, transtype, amount, balance, timestamp) VALUES (%s, %s, %s, %s, CURRENT_TIMESTAMP)"
